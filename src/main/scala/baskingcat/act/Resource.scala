...
object Resource {
  ...
  lazy val imageDataMap: Map[String, ImageData] = Map(
  ...
  lazy val textures = Map(
    "negi" -> Textures.load("/home/halcat/projects/act/src/main/resources/items/negi.png"),
    "title" -> Textures.load(path + "backgrounds/title.png"),
    "background" -> Textures.load(path + "backgrounds/background.png"),
    "clear" -> Textures.load(path + "backgrounds/clear.png"))

  def destroy() {
    textures.values foreach (_.delete())
  }
