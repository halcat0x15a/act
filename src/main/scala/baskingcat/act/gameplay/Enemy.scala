...
case class Enemy[A <: Status, B <: Direction](status: A, direction: B, bounds: Rectangle[Float], velocity: Vector2D[Float], life: Int) extends GameplayObject with HasStatus[A] with HasDirection[B] with Live[A] with Movable[A, B] with Walkable[A, B] {
  ...
  def walk[C <: Direction](direction: C)(implicit stage: Stage) = {
    ...
    copy(status = s, direction = direction, velocity = mzero[Vector2D[Float]])
  }
  ...
  def damaged(implicit stage: Stage) = {
    ...
    copy(status = s, life = stage.objects.any(detect).fold(life - 1, life))
  }
